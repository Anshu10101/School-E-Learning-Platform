...
if($submit=="Submit")
{
	$roww = mysql_query("select * from subtopic1 where class ='".$classask."' && subject =upper('".$subjectask."') && topic =upper('".$topicask."') && subtopic =upper('".$sub."')");
	$sk = mysql_affected_rows();
	if($sk=="0")
	{
			mysql_query("insert into subtopic1 values ('','".$classask."',upper('".$subjectask."'),
			upper('".$topicask."'),upper('".$sub."'))");
			$k = mysql_affected_rows($link);
				if($k>0)
					{$sub = strtoupper($sub);
					 mkdir("../vedio/".$classask."/".$subjectask."/".$topicask."/".$sub."");
