...
if($submit=="Submit")
{
	$roww = mysql_query("select * from asksubtopic where class ='".$classask."' && subject 
	=upper('".$subjectask."') && topic =upper('".$topicask."')");
	$sk = mysql_affected_rows();
	if($sk=="0")
	{
			mysql_query("insert into asksubtopic values ('','".$classask."',upper('".$subjectask."'),
			upper('".$topicask."'),upper('".$subtopicask."'))");
		...
		while($row = mysql_fetch_array($roww))
		{
		  if($row['nosubtopic']=="$subtopicask")
			...
			echo("<script>alert('This subtopic ia already added in list !');</script>");			
			  ...
			  if($subtopicask=="NO")
			    {					
				mysql_query("update asksubtopic set nosubtopic ='NO'  
				where class ='".$classask."' && subject=upper('".$subjectask."') && 
				topic =upper('".$topicask."')");
			  ...
			  if($subtopicask=="YES")
				{ 	
				mysql_query("update asksubtopic set nosubtopic ='YES'  
				where class ='".$classask."' && subject=upper('".$subjectask."') && 
				topic =upper('".$topicask."')");
				$s = mysql_affected_rows($link);
				if($s>0)
