...
<script>
    history.pushState(null, null, 'logclass.php');
...
if($submit == "Add")
{
$roww = mysql_query("select * from subject where class ='".$class."' && subject =upper('".$nsub."')");
$n = mysql_affected_rows();
	if($n=="0")
	{
	mysql_query("insert into subject values ('','".$class."',upper('".$nsub."'))");
	$s = mysql_affected_rows($link);
		if($s>0)
			{$nsub = strtoupper($nsub);
			 mkdir("vedio/".$class."/".$nsub."");
	...
	{
	echo("<script>alert('This subject ia already added in ".$class." subjects list !');</script>");
	}
}
                ...
                <div class="mbr-box__magnet mbr-box__magnet--top-left mbr-section__left col-sm-8">
                    <div class="mbr-section__container">
                        <div class="mbr-header mbr-header--auto-align mbr-header--wysiwyg">
							...
							<a href="loghome.php" style="color:#CC3300;">HOME</a> >> Add new subject</h5>
                        </div>
                    </div>
                </div>
            </div>
...
?>
                            ...
                            <div class="col-xs-12 col-md-6">
                                <div class="form-group">
									<label class="form-control-label" for="form1-9-name">Subject Name<span class="form-asterisk">*</span></label> 
	<input type="text" name="nsub" class="form-control" required="" data-form-field="Name" id="form1-9-name" />
								</div>
                            </div>
